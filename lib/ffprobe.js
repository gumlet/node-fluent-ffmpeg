...
function parseFfprobeOutput(out) {
  ...
  var lineIndex = 0;
  function nextLine() {
    return lineIndex < lines.length ? lines[lineIndex++] : undefined;
  }
    ...
    this._getFfprobePath((err, path) => {
      ...
      function handleExit(err) {
        ...
        if (processExited && stdoutClosed && stderrClosed) {
          ...
          // Process output
          const data = parseFfprobeOutput(stdoutChunks.join(''));

          // Handle legacy output with "TAG:x" and "DISPOSITION:x" keys
          [data.format].concat(data.streams).forEach((target) => {
            if (target) {
              ...
              if (legacyTagKeys.length) {
                ...
                legacyTagKeys.forEach((tagKey) => {
                  target.tags[tagKey.slice(4)] = target[tagKey];
                  delete target[tagKey];
                });
              }

              const legacyDispositionKeys = Object.keys(target).filter(legacyDisposition);

              if (legacyDispositionKeys.length) {
                target.disposition = target.disposition || {};

                legacyDispositionKeys.forEach((dispositionKey) => {
      ...
      // Handle stdout/stderr streams (chunk arrays avoid repeated string reallocation)
