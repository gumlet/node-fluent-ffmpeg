...
export default (proto) => {
  ...
  proto._getArguments = function() {
    ...
    var fileOutput = this._outputs.some((output) => output.isFile);

    return [].concat(
        // Inputs and input options
        this._inputs.reduce((args, input) => {
        ...
        this._outputs.reduce((args, output) => {
  ...
  proto._prepare = function(callback, readMetadata) {
    ...
    new Promise((resolve, reject) => {
      self._checkCapabilities((err) => err ? reject(err) : resolve());
    })
      .then(() => {
        if (!readMetadata) return;
        return new Promise((resolve) => {
          self.ffprobe(0, (err, data) => {
      ...
      .then(() => {
        const flvmeta = self._outputs.some((output) => {
        ...
        if (flvmeta) {
          return new Promise((resolve, reject) => {
            self._getFlvtoolPath((err) => err ? reject(err) : resolve());
          });
        }
      })
      .then(() => {
      ...
      .then((args) => {
        return new Promise((resolve, reject) => {
          self.availableEncoders((err, encoders) => {
  ...
  proto.run = function() {
    ...
    var outputPresent = this._outputs.some((output) => 'target' in output);
    ...
    var outputStream = this._outputs.filter((output) => typeof output.target !== 'string')[0];

    // Get input stream if any
    var inputStream = this._inputs.filter((input) => typeof input.source !== 'string')[0];
    ...
    self._prepare((err, args) => {
      ...
      self._spawnFfmpeg(
        ...
        function processCB(ffmpegProc, stdoutRing, stderrRing) {
          ...
          if (inputStream) {
            inputStream.source.on('error', (err) => {
          ...
          if (outputStream) {
            ...
            outputStream.target.on('error', (err) => {
        ...
        function endCB(err, stdoutRing, stderrRing) {
          ...
          if (err) {
            ...
            const flvmeta = self._outputs.filter((output) => output.flags.flvmeta);

            if (flvmeta.length) {
              self._getFlvtoolPath((err, flvtool) => {
                if (err) {
                  return emitEnd(err);
                }

                Promise.all(flvmeta.map((output) => {
                  return new Promise((resolve, reject) => {
                    spawn(flvtool, ['-U', output.target], {windowsHide: true})
                      .on('error', (err) => {
                        reject(new Error(`Error running ${flvtool} on ${output.target}: ${err.message}`));
                      })
                      .on('exit', (code, signal) => {
                        if (code !== 0 || signal) {
                          reject(
                  ...
                  .then(() => emitEnd(null, stdoutRing.get(), stderrRing.get()))
                  .catch((err) => emitEnd(err));
              });
            } else {
  ...
  proto.renice = function(niceness) {
    if (!utils.isWindows) {
      ...
      if (this.ffmpegProc) {
        ...
        const renice = spawn('renice', [niceness, '-p', pid], {windowsHide: true});
