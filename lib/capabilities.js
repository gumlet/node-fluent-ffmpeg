    ...
    async.waterfall([
      ...
      (cb) => {
        if (process.env.FFMPEG_PATH) {
          fs.exists(process.env.FFMPEG_PATH, (exists) => {
            if (exists) {
    ...
    async.waterfall([
      ...
      (cb) => {
        if (process.env.FFPROBE_PATH) {
          fs.exists(process.env.FFPROBE_PATH, (exists) => {
    ...
    async.waterfall([
      ...
      (cb) => {
        if (process.env.FLVMETA_PATH) {
          fs.exists(process.env.FLVMETA_PATH, (exists) => {
      ...
      (flvtool, cb) => {
        ...
        if (process.env.FLVTOOL2_PATH) {
          fs.exists(process.env.FLVTOOL2_PATH, (exists) => {
      ...
      (flvtool, cb) => {
        ...
        utils.which('flvmeta', (err, flvmeta) => {
          cb(err, flvmeta);
        });
      },
      ...
      (flvtool, cb) => {
        ...
        utils.which('flvtool2', (err, flvtool2) => {
          cb(err, flvtool2);
        });
      },
    ], (err, flvtool) => {
