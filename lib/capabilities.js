...
export default (proto) => {
   ...
   * @private
   */
  proto._getFfmpegPath = async (callback) => {
    if ('ffmpegPath' in cache) {
      return callback(null, cache.ffmpegPath);
    }

    let finalFFmpegpath, error;

    if (process.env.FFMPEG_PATH) {
      try {
        await fs.access(process.env.FFMPEG_PATH, fs.constants.F_OK)
        finalFFmpegpath = process.env.FFMPEG_PATH;
      } catch {
        // FFMPEG_PATH is incorrect, continue to search
      }
    }

    if(!finalFFmpegpath) {
      finalFFmpegpath = await new Promise((resolve) => {
        utils.which('ffmpeg', (err, ffmpeg) => {
          if (err) {
            error = err
          } else {
            resolve(ffmpeg);
    ...
    if(error) {
      ...
      cache.ffmpegPath = finalFFmpegpath || '';
   ...
   * @private
   */
  proto._getFfprobePath = async (callback) => {

    if ('ffprobePath' in cache) {
      return callback(null, cache.ffprobePath);
    }

    let finalFFprobePath, error;

    if (process.env.FFPROBE_PATH) {
      try {
        await fs.access(process.env.FFPROBE_PATH, fs.constants.F_OK)
        finalFFprobePath = process.env.FFPROBE_PATH;
      } catch {
        // FFPROBE_PATH is incorrect, continue to search
      }
    }

    if(!finalFFprobePath) {
      finalFFprobePath = await new Promise((resolve) => {
        utils.which('ffprobe', (err, ffprobe) => {
          if (err) {
            error = err
          } else {
            resolve(ffprobe);
    ...
    if(!finalFFprobePath) {
      ...
      this._getFfmpegPath((err, ffmpeg) => {
          if (err) {
            ...
            fs.access(ffprobe, fs.constants.F_OK, (err) => {
              resolve(!err ? ffprobe : '');
            });
          } else {
            resolve('');
    ...
    if(error) {
      ...
      cache.ffprobePath = finalFFprobePath || '';
   ...
   * @private
   */
   proto._getFlvtoolPath = async (callback) => {
    if ('flvtoolPath' in cache) {
      return callback(null, cache.flvtoolPath);
    }

    let finalFlvtoolPath, error;

    if(process.env.FLVMETA_PATH) {
      try {
        await fs.promises.access(process.env.FLVMETA_PATH, fs.constants.F_OK)
        finalFlvtoolPath = process.env.FLVMETA_PATH;
      } catch {
        // FLVMETA_PATH is not accessible, continue
      }
    }

    if(!finalFlvtoolPath && process.env.FLVTOOL2_PATH) {
      try {
        await fs.promises.access(process.env.FLVTOOL2_PATH, fs.constants.F_OK)
        finalFlvtoolPath = process.env.FLVTOOL2_PATH;
      } catch {
        // FLVTOOL2_PATH is not accessible, continue
      }
    }

    if(!finalFlvtoolPath) {
      finalFlvtoolPath = await new Promise((resolve) => {
        utils.which('flvmeta', (err, path) => {
          if(err) {
            error = err;
          }
          resolve(path || '');
    ...
    if(error) {
      ...
      cache.flvtoolPath = finalFlvtoolPath;
  ...
  proto._checkCapabilities = function(callback) {
    ...
    new Promise((resolve, reject) => {
      self.availableFormats((err, formats) => err ? reject(err) : resolve(formats));
    })
      .then((formats) => {
        let unavailable;

        // Output format(s)
        unavailable = self._outputs.reduce((fmts, output) => {
        ...
        // Input format(s)
        unavailable = self._inputs.reduce((fmts, input) => {
      ...
      .then(() => {
        return new Promise((resolve, reject) => {
          self.availableEncoders((err, encoders) => err ? reject(err) : resolve(encoders));
        });
      })
      .then((encoders) => {
        let unavailable;
        ...
        // Audio codec(s)
        unavailable = self._outputs.reduce((cdcs, output) => {
