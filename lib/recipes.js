...
module.exports = function recipes(proto) {
  ...
  proto.screenshots = function(config, folder) {
    ...
    async.waterfall([
      ...
      function computeTimemarks(next) {
        if (config.timemarks.some((t) => ('' + t).match(/^[\d.]+%$/))) {
          ...
          getMetadata((err, meta) => {
            if (err) {
              ...
              var vstream = meta.streams.reduce((biggest, stream) => {
      ...
      function replaceVariableTokens(pattern, next) {
        var filenames = config.timemarks.map((t, i) => pattern
      ...
      function createDirectory(filenames, next) {
        fs.exists(config.folder, (exists) => {
          if (!exists) {
      ...
      if ('size' in config) {
        ...
        var sizeFilters =  self._currentOutput.sizeFilters.get().map((f, i) => {
          if (i > 0) {
            f.inputs = 'size' + (i - 1);
          }
  ...
  proto.concat = function(target, options) {
    ...
    this.ffprobe(this._inputs.indexOf(fileInput), (err, data) => {
      ...
      var hasAudioStreams = data.streams.some((stream) => stream.codec_type === 'audio');

      var hasVideoStreams = data.streams.some((stream) => stream.codec_type === 'video');
